...
In this paper we will explore problems often encountered when solving differential numerically on a computer, by looking at a one-dimensional Poisson equation often encountered in physics. We will explore how efficient matrix algorithms like the Thomas algorithm (MORTENS KOMPENDIUM) and LU-decomposition as shown by \citep[p.142]{lay:2015} and \citep[p.668]{Boyd:2004}, as oppose to standard Gaussian elimination, can be used for solving ordinary differential equations (ODEs). Since these algorithms only are approximations, understanding the numerical errors is important. Thus we will also look at how the error behaves when choosing a specific grid size and when truncation errors become evident.   
